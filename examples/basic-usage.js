...
async function main() {
  ...
  const originalFile = new TextEncoder().encode('Hello, World! This is a secret message. 🔒');
  console.log(`   ✓ File size: ${originalFile.length} bytes`);
  console.log(`   ✓ Content: "${new TextDecoder().decode(originalFile)}"\n`);
  ...
  const decryptedContent = new TextDecoder().decode(decryptedFile);
  const matches = decryptedContent === new TextDecoder().decode(originalFile);
